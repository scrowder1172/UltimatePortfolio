...
final class ExtensionTests: BaseTestCase {
    ...
    func testIssueTitleUnwrap() {
        ...
        XCTAssertEqual(issue.issueTitle, "Updated issue", "Changing issueTitle should also change title.")
    }
    ...
    func testIssueContentUnwrap() {
        ...
        XCTAssertEqual(issue.issueContent, "Updated issue", "Changing issueContent should also change content.")
    }
    ...
    func testIssueTagsUnwrap() {
        ...
        issue.addToTags(tag)
        XCTAssertEqual(issue.issueTags.count, 1, "Adding one tag to an issue should result in issue tags having count 1.")
    }
    ...
    func testIssueSortingIsStable() {
        ...
        let allIssues: [Issue] = [issue1, issue2, issue3]
...
//        XCTAssertEqual([issue3, issue1, issue2], sorted, "Sorting issue arrays should use name then creation date.")
        XCTAssertEqual([issue1, issue2, issue3], sorted, "Sorting issue arrays should use name then creation date.")
    ...
    func testTagSortingIsStable() {
        ...
        tag2.name = "B Tag"
        tag2.id = UUID(uuidString: "FFFFFFFF-FFFF-4536-B53A-55F1B0B895A1")
        
        let tag3 = Tag(context: managedObjectContext)
        tag3.name = "A Tag"
        tag3.id = UUID()
        
        let allTags: [Tag] = [tag1, tag2, tag3]
